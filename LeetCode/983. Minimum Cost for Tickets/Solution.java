 ...
 * The size of the dp array depends on the last travel day, 
 * so we don't need an array length to be 365.
 * 
 * We do need a boolean array to mark the travel days, 
 * the reason is if it is not a travel day we don't need a ticket. 
 * 
...
class Solution {
    public int mincostTickets(int[] days, int[] costs) {
        ...
        for (int i = 1; i < 366; i++) 
        {
            ...
            int min = dp[i - 1] + costs[0];                             // 1 day
            min = Math.min(min, (i < 7 ? 0 : dp[i - 7]) + costs[1]);    // 7 day
            min = Math.min(min, (i < 30 ? 0 : dp[i - 30]) + costs[2]);  // 30 day
            dp[i] = min;
        }
        return dp[365];
    }
